    ...
    class WrappedPCell(object):
        def __init__(self, *args, **kwargs):
            raise AttributeError(
                "WrappedPCell requires a working installation of klayout python package"
            )

    class WrappedLibrary(object):
        def __init__(self, *args, **kwargs):
            raise AttributeError(
                "WrappedLibrary requires a working installation of klayout python package"
            )
    ...
    class WrappedPCell(pya.PCellDeclarationHelper):
        ...
        def kwargs_to_params(self):
            ...
            def kwarg_to_param(key, default=None):
                self.param(
                    key, pytype_to_PCelltype(type(default)), key, default=default
                )
    ...
    class WrappedLibrary(pya.Library):
        ...
        def __init__(self):
            ...
            for func in self.all_funcs_to_wrap:
                if callable(func) and isinstance(func, functools.partial):
                    while hasattr(func, "func"):
                        func = func.func

                function_name = func.__name__
                self.layout().register_pcell(function_name, WrappedPCell(func))
    ...
    def pfunc(*args, **kwargs):
        for k in kwargs.keys():
            if k not in all_kwargs.keys():
                raise ValueError(
                    'Invalid argument. "{}" is not in {}'.format(k, all_kwargs.keys())
                )
